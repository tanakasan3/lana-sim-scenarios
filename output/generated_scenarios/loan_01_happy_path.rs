...
#[instrument(
    name = "sim_bootstrap.loan_01_happy_path_scenario",
    skip(app, clock, clock_ctrl),
    err
)]
pub async fn loan_01_happy_path_scenario(
    sub: Subject,
    app: &LanaApp,
    clock: &ClockHandle,
    clock_ctrl: &ClockController,
) -> anyhow::Result<()> {
    event!(tracing::Level::INFO, "Starting loan_01_happy_path scenario");
    ...
    // Initiate disbursal
    app.credit()
        .initiate_disbursal(&sub, cf_id, UsdCents::try_from_usd(dec!(5000.0))?)
        .await?;
    ...
    event!(tracing::Level::INFO, "Completed loan_01_happy_path scenario");
    Ok(())
}