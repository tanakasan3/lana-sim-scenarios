//! Scenario: loan_09_multiple_disbursals
//! 
...
#[instrument(
    name = "sim_bootstrap.loan_09_multiple_disbursals_scenario",
    skip(app, clock, clock_ctrl),
    err
)]
pub async fn loan_09_multiple_disbursals_scenario(
    sub: Subject,
    app: &LanaApp,
    clock: &ClockHandle,
    clock_ctrl: &ClockController,
) -> anyhow::Result<()> {
    event!(tracing::Level::INFO, "Starting loan_09_multiple_disbursals scenario");
    ...
    let cf_terms = helpers::std_terms();  // TODO: customize terms from {'annual_rate': '0.10', 'duration': {'Months': 12}, 'interest_due_duration_from_accrual': {'Days': 30}, 'accrual_cycle_interval': 'EndOfMonth', 'accrual_interval': 'EndOfDay', 'one_time_fee_rate': '0.01', 'liquidation_cvl': {'Finite': '1.10'}, 'margin_call_cvl': {'Finite': '1.25'}, 'initial_cvl': {'Finite': '1.40'}, 'disbursal_policy': 'MultipleDisbursal'}
    let cf_amount = UsdCents::try_from_usd(dec!(20000.0))?;
    let cf_proposal = app
    ...
    // Initiate disbursal
    app.credit()
        .initiate_disbursal(&sub, cf_id, UsdCents::try_from_usd(dec!(10000.0))?)
        .await?;
    ...
    // Initiate disbursal
    app.credit()
        .initiate_disbursal(&sub, cf_id, UsdCents::try_from_usd(dec!(6000.0))?)
        .await?;
    ...
    // Initiate disbursal
    app.credit()
        .initiate_disbursal(&sub, cf_id, UsdCents::try_from_usd(dec!(4000.0))?)
        .await?;
    ...
    event!(tracing::Level::INFO, "Completed loan_09_multiple_disbursals scenario");
    Ok(())
}