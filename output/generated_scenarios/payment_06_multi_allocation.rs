//! Scenario: payment_06_multi_allocation
//! 
...
#[instrument(
    name = "sim_bootstrap.payment_06_multi_allocation_scenario",
    skip(app, clock, clock_ctrl),
    err
)]
pub async fn payment_06_multi_allocation_scenario(
    sub: Subject,
    app: &LanaApp,
    clock: &ClockHandle,
    clock_ctrl: &ClockController,
) -> anyhow::Result<()> {
    event!(tracing::Level::INFO, "Starting payment_06_multi_allocation scenario");
    ...
    // Record payment
    app.record_payment_with_date(&sub, cf_id, UsdCents::try_from_usd(dec!(91.67))?, clock.today()).await?;

    event!(tracing::Level::INFO, "Completed payment_06_multi_allocation scenario");
    Ok(())
}