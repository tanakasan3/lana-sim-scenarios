...
#[instrument(
    name = "sim_bootstrap.payment_03_defaulted_scenario",
    skip(app, clock, clock_ctrl),
    err
)]
pub async fn payment_03_defaulted_scenario(
    sub: Subject,
    app: &LanaApp,
    clock: &ClockHandle,
    clock_ctrl: &ClockController,
) -> anyhow::Result<()> {
    event!(tracing::Level::INFO, "Starting payment_03_defaulted scenario");
    ...
    let cf_terms = helpers::std_terms();  // TODO: customize terms from {'annual_rate': '0.10', 'duration': {'Months': 6}, 'interest_due_duration_from_accrual': {'Days': 15}, 'obligation_overdue_duration_from_due': {'Days': 15}, 'obligation_liquidation_duration_from_due': {'Days': 30}, 'accrual_cycle_interval': 'EndOfMonth', 'accrual_interval': 'EndOfDay', 'one_time_fee_rate': '0.01', 'liquidation_cvl': {'Finite': '1.10'}, 'margin_call_cvl': {'Finite': '1.25'}, 'initial_cvl': {'Finite': '1.40'}, 'disbursal_policy': 'SingleDisbursal'}
    ...
    event!(tracing::Level::INFO, "Completed payment_03_defaulted scenario");
    Ok(())
}