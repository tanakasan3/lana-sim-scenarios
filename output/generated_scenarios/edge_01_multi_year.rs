...
#[instrument(
    name = "sim_bootstrap.edge_01_multi_year_scenario",
    skip(app, clock, clock_ctrl),
    err
)]
pub async fn edge_01_multi_year_scenario(
    sub: Subject,
    app: &LanaApp,
    clock: &ClockHandle,
    clock_ctrl: &ClockController,
) -> anyhow::Result<()> {
    event!(tracing::Level::INFO, "Starting edge_01_multi_year scenario");
    ...
    let cf_terms = helpers::std_terms();  // TODO: customize terms from {'annual_rate': '0.08', 'duration': {'Months': 24}, 'interest_due_duration_from_accrual': {'Days': 30}, 'accrual_cycle_interval': 'EndOfMonth', 'accrual_interval': 'EndOfDay', 'one_time_fee_rate': '0.005', 'liquidation_cvl': {'Finite': '1.10'}, 'margin_call_cvl': {'Finite': '1.25'}, 'initial_cvl': {'Finite': '1.40'}, 'disbursal_policy': 'SingleDisbursal'}
    ...
    event!(tracing::Level::INFO, "Completed edge_01_multi_year scenario");
    Ok(())
}