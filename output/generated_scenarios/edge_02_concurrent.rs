...
#[instrument(
    name = "sim_bootstrap.edge_02_concurrent_scenario",
    skip(app, clock, clock_ctrl),
    err
)]
pub async fn edge_02_concurrent_scenario(
    sub: Subject,
    app: &LanaApp,
    clock: &ClockHandle,
    clock_ctrl: &ClockController,
) -> anyhow::Result<()> {
    event!(tracing::Level::INFO, "Starting edge_02_concurrent scenario");
    ...
    // Create facility proposal
    let cf_terms = helpers::std_terms();  // TODO: customize terms from {'annual_rate': '0.12', 'duration': {'Months': 12}, 'interest_due_duration_from_accrual': {'Days': 30}, 'accrual_cycle_interval': 'EndOfMonth', 'accrual_interval': 'EndOfDay', 'one_time_fee_rate': '0.015', 'liquidation_cvl': {'Finite': '1.10'}, 'margin_call_cvl': {'Finite': '1.25'}, 'initial_cvl': {'Finite': '1.40'}, 'disbursal_policy': 'SingleDisbursal'}
    let cf_amount = UsdCents::try_from_usd(dec!(7500.0))?;
    let cf_proposal = app
        .create_facility_proposal(&sub, customer_1_id, cf_amount, cf_terms, None::<CustodianId>)
        .await?;
    let proposal_id = cf_proposal.id;
    ...
    event!(tracing::Level::INFO, "Completed edge_02_concurrent scenario");
    Ok(())
}