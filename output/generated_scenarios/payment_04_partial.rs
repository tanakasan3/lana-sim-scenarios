...
#[instrument(
    name = "sim_bootstrap.payment_04_partial_scenario",
    skip(app, clock, clock_ctrl),
    err
)]
pub async fn payment_04_partial_scenario(
    sub: Subject,
    app: &LanaApp,
    clock: &ClockHandle,
    clock_ctrl: &ClockController,
) -> anyhow::Result<()> {
    event!(tracing::Level::INFO, "Starting payment_04_partial scenario");
    ...
    // Record payment
    app.record_payment_with_date(&sub, cf_id, UsdCents::try_from_usd(dec!(20.0))?, clock.today()).await?;
    // Advance time: 5 days
    for _ in 0..5 {
        clock_ctrl.advance(ONE_DAY).await;
    }
    ...
    // Record payment
    app.record_payment_with_date(&sub, cf_id, UsdCents::try_from_usd(dec!(21.67))?, clock.today()).await?;

    event!(tracing::Level::INFO, "Completed payment_04_partial scenario");
    Ok(())
}